...
| フェーズ | タイトル | 状態 | 重点領域 |
|---|---|---|---|
| Phase 1 | 基盤整備と UserProfile 連携 | 進行中 (70%) | バックエンド基盤、認証、パーソナライズ計算 |
...
1. **UserProfile API の実装 (T-05)**: 認証ユーザーが自身の楽天ランクやカード情報を保存できるようにする。
2. **ポイント算出ロジックの実装 (T-06)**: 楽天 SPU や Yahoo プレミアム、カード加算などの複雑な計算をバックエンドに集約する。
3. **検索 API のパーソナライズ (T-08)**: 検索結果に、ユーザーごとの `points` / `effectivePrice` を載せて返す。
4. **型同期パイプラインの構築 (T-09)**: API 仕様とフロントエンドの型のズレを自動的に防ぐ。
