...
Phase 1 のタスク（T-01〜T-09 / B-3）はすべて完了。次のアクションは Phase 2 の着手。

1. **価格履歴 API の実装 (F-16)**: `PriceHistory` テーブルから JAN / EcSiteProduct 単位で価格推移を返す API を実装する。
2. **価格推移チャート UI (F-17)**: フロントエンドに価格推移チャートを実装し、現在の価格の妥当性をユーザーが判断できるようにする。
