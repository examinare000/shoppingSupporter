...
#### 4.2. 価格推移シナリオ（ルールベース・初期実装）

ML を使わず、以下のルールで「N 日後の想定ベース価格」を推定する:

- **直近 30/90 日の最安値・中央値**を `PriceHistory` から算出
- セール期間中は **過去同種セール時の中央値** を採用（例: 過去のお買い物マラソン中の同 EcSiteProduct 平均値）
- 履歴が不足する商品（30 日未満のデータ）は **予測を返さず**、`forecast_available: false` を立てる（信頼度の偽装を避ける）
