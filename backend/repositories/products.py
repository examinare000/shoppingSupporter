...
def _searchable_text() -> ColumnElement[str]:
    # Must match the trigram GIN index expression in 0002 migration so
    # Postgres can use the index instead of falling back to a sequential
    # scan. `immutable_array_to_string` is the wrapper introduced in 0002
    # because the built-in `array_to_string` is STABLE (not IMMUTABLE) and
    # therefore cannot be used inside an expression index.
    space = literal(" ")
    return (
