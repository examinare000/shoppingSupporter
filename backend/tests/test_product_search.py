...
- Repository unit tests against `search_products(db, params)` in
  `repositories.products`. These exercise FTS, trigram fuzzy match,
...
The endpoint contract differs from the legacy `/api/products/search`
implementation in this repo (which was authored for an earlier order.md):
- New: validation failures return 422 (legacy returned 200 + []).
- New: response is a `{items, page, totalPages, totalCount}` envelope
  (legacy returned a bare array).
- New: query params are `q`, `inStock`, `priceMin`, `priceMax`, `sort`,
  `page` (legacy used `q`, `limit`).
"""
...
from repositories.products import SearchParams, search_products
...
class TestEndpointValidationErrors:
    ...
    The legacy `/api/products/search` converted every error path to 200+[].
    The new contract returns FastAPI's standard 422 so callers can detect
    bad input rather than mistaking it for an empty result set.
    """
